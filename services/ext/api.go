...
func (api *PublicAPI) RegisterForPushNotifications(ctx context.Context, deviceToken string, apnTopic string, tokenType protobuf.PushNotificationRegistration_TokenType) error {
	...
	err := api.service.accountsDB.SaveSetting(settings.RemotePushNotificationsEnabled.GetReactName(), true)
	if err != nil {
		return err
	}
	err = api.service.accountsDB.SaveSetting(settings.NotificationsEnabled.GetReactName(), true)
	if err != nil {
		return err
	}
...
func (api *PublicAPI) UnregisterFromPushNotifications(ctx context.Context) error {
	err := api.service.accountsDB.SaveSetting(settings.RemotePushNotificationsEnabled.GetReactName(), false)
	if err != nil {
		return err
	}
	err = api.service.accountsDB.SaveSetting(settings.NotificationsEnabled.GetReactName(), false)
	if err != nil {
		return err
	}
